...
def dataframe_burst_char(mean_burst_duration_M1,burst_amplitude_M1,burst_rate_M1):
    ...
    '''
    pdburst = pd.DataFrame({'Subject':1, 'Medication':'ON','Mean Burst Duration (s)':mean_burst_duration_M1,'Mean Burst Amplitude (au)':burst_amplitude_M1,'Burst Rate': burst_rate_M1} ,index=['Run 1'])
    return pdburst
...
def dataframe_burst_dynamics(norm_histogram_duration):
    ...
    df_his = df_probdur_t_M1.assign( Subject= 1 , Medication = 'ON', Run= 1 )
    return df_his

def dataframe_npow(power_spectra_norm):
    pw = pd.DataFrame(power_spectra_norm)
    pw_t = pw.T
    pw_a = pw_t.assign( Subject= 1, Medication = 'ON', Run= 1 )
    return pw_a
